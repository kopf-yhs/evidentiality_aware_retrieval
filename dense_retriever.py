...
def generate_question_vectors(
    ...
    n = len(questions)
    query_vectors = []
    # Extra variables for attention outputs
    attentions = []
    question_ids = []
    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            if selector:
                ...
                _, out, hidden, attn = BiEncoder.get_representation(
                    question_encoder,
                    q_ids_batch,
                    q_seg_batch,
                    q_attn_mask,
                ...
                _, out, hidden, attn = question_encoder(
                    q_ids_batch,
                    q_seg_batch, 
                    q_attn_mask, 
                    output_hidden_states=output_hidden_states,
            ...
            # For stacking attentions from multiple batches
            if attn:
                attn = torch.stack(attn, dim=0)
                attn = torch.transpose(attn, 0, 1).cpu()

            query_vectors.extend(out.cpu().split(1, dim=0))
            attentions.append(attn)
            question_ids.extend(q_ids_batch.cpu().split(1, dim=0))
            question_attention_masks.extend(q_attn_mask.cpu().split(1, dim=0))

            if len(query_vectors) % 100 == 0:
                logger.info("Encoded queries %d", len(query_vectors))

    query_tensor = torch.cat(query_vectors, dim=0)
    question_ids = torch.cat(question_ids, dim=0)
    question_attention_masks = torch.cat(question_attention_masks, dim=0)
    ...
    return query_tensor, attentions, question_ids, question_attention_masks
...
class DenseRetriever(object):
    ...
    def generate_question_vectors(self, 
        questions: List[str], 
        query_token: str = None, 
        output_hidden_states: bool = False,
        output_attentions: bool = False) -> T:
...
def validate(
    ...
    logger.info("Validation results: top 5 accuracy %s", top_k_hits[4])
    logger.info("Validation results: top 20 accuracy %s", top_k_hits[19])
    logger.info("Validation results: top 100 accuracy %s", top_k_hits[99])
...
def save_attentions(
    ...
    #attention_masks = attention_masks.detach().to('cpu')

    '''special_tok_masks = list()
    for qids in token_ids:
        special_tok_masks.append(torch.tensor(tensorizer.get_special_tokens_mask(qids), dtype=bool))
    special_tok_masks = torch.stack(special_tok_masks, dim=0)
    all_valid_tokens = (~special_tok_masks & attention_masks)'''
    
    tensor_list = list()
    attention_list = list()
    for qid, que, tok, tens in zip(
        question_ids, 
        questions,
        token_ids,
        questions_tensor,
        ...
        tensor_list.append((qid, tens.view(-1).numpy()))
        attention_list.append([
            qid, 
            que, 
            tensorizer.convert_ids_to_tokens(tok), 
            tens, 
        ])

    with open('test_embeddings.pkl_0','wb') as q_tensor_file:
        pickle.dump(tensor_list, q_tensor_file)

    #with open('attentions.pkl','wb') as q_attn_file:
    #    pickle.dump(attention_list, q_attn_file)

    #max_attn_index = torch.argmax(valid_mean_cls_attentions, dim=1)
    #question_tokens = [tensorizer.convert_ids_to_tokens(qid) for qid in token_ids]
    #for toks, idx in zip(question_tokens, max_attn_index):
    #    print(toks[idx])
...
def main(cfg: DictConfig):
    ...
    logger.info('Get dataset %s' % cfg.qa_dataset)
    ...
    questions_tensor, question_attentions, q_token_ids, q_attn_mask = retriever.generate_question_vectors(
        questions,
    ...
    if cfg.rpc_retriever_cfg_file and cfg.rpc_index_id:
        ...
        id_prefixes = []
        ctx_sources = []
        if type(cfg.ctx_datasets) == str:
            context_set = [cfg.ctx_datasets]
        else:
            context_set = cfg.ctx_datasets
        for ctx_src in context_set:
