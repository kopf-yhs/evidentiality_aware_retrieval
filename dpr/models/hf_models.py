...
def get_bert_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    ...
    ''' TODO: Part of SPAR Implementation
        if cfg.bm25_model_file:
        dropout = cfg.bm25_encoder.dropout if hasattr(cfg.bm25_encoder, "dropout") else 0.0
        question_encoder = HFBertEncoder.init_encoder(
            cfg.bm25_encoder.pretrained_model_cfg,
            projection_dim=cfg.bm25_encoder.projection_dim,
            dropout=dropout,
            pretrained=cfg.bm25_encoder.pretrained,
            **kwargs
        )
        ctx_encoder = HFBertEncoder.init_encoder(
            cfg.bm25_encoder.pretrained_model_cfg,
        ...
        fix_ctx_encoder = cfg.bm25_encoder.fix_ctx_encoder if hasattr(cfg.bm25_encoder, "fix_ctx_encoder") else False
        biencoder = BiEncoder(question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder)
    '''
    optimizer = (
        get_optimizer(
...
class HFBertEncoder(BertModel):
    ...
    def forward(
        ...
        if transformers.__version__.startswith("4") and isinstance(
            ...
            if output_attentions:
                sequence_output, pooled_output, hidden_states, attentions = out
            else:
...
class BertTensorizer(Tensorizer):
    ...
    def get_attn_mask(self, tokens_tensor: T) -> T:
        attn_mask = (tokens_tensor != self.get_pad_id()) | (tokens_tensor != self.get_mask_id())
        return attn_mask
    ...
    def get_token_id(self, token: str) -> int:
        return self.tokenizer.vocab[token] if token in self.tokenizer.vocab else -1

    def get_punctuation_mask(self, token_ids):
        return [tok in string.punctuation for tok in self.tokenizer.convert_ids_to_tokens(token_ids)]
