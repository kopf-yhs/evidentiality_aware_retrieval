...
from mdr.retrieval.criterions import (mhop_eval, mhop_loss, mhop_loss_cf)
from mdr.retrieval.data.mhop_dataset import MhopDataset, CfMhopDataset, mhop_collate, mhop_collate_cf
...
def main():
    ...
    if args.counterfactual:
        cf_collate_fc = partial(mhop_collate_cf, pad_id=tokenizer.pad_token_id)
    ...
    if args.do_train:
        ...
        if args.counterfactual:
            train_dataset = CfMhopDataset(tokenizer, args.train_file, args.max_q_len, args.max_q_sp_len, args.max_c_len, train=True)
            loss_function = mhop_loss_cf
            train_dataloader = DataLoader(train_dataset, batch_size=args.train_batch_size, pin_memory=True, collate_fn=cf_collate_fc, num_workers=args.num_workers, shuffle=True)
        else:
            train_dataset = MhopDataset(tokenizer, args.train_file, args.max_q_len, args.max_q_sp_len, args.max_c_len, train=True)
            loss_function = mhop_loss
        ...
        for epoch in range(int(args.num_train_epochs)):
            for batch in tqdm(train_dataloader):
                ...
                loss = loss_function(model, batch, args)
