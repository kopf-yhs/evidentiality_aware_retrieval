...
def get_chn_mapping(positive_index : list, cf_negative_index : list, shape : torch.Size, device):
        n_questions = shape[0]
        n_scores = shape[1]
        counterfactual_mapping = [[i != cf_negative_index[j] and i != positive_index[j] for i in range(n_scores)] for j in range(n_questions)]
        return torch.tensor(counterfactual_mapping).to(device)

def get_cpp_mapping(positive_index : list, shape : torch.Size, device):
    n_questions = shape[0]
    n_scores = shape[1]
    masked_mapping = [[i == positive_index[j] for i in range(n_scores)] for j in range(n_questions)]
    return torch.tensor(masked_mapping).to(device)
...
def AADPRLoss(
        ...
        chn_mask = get_chn_mapping(labels, cf_labels, all_scores.shape, all_scores.device)
        cpp_mask = get_cpp_mapping(labels, all_scores.shape, all_scores.device)

        chn_scores = all_scores.float().masked_fill(chn_mask.bool(), float('-inf')).type_as(all_scores)
        cpp_scores = all_scores.float().masked_fill(cpp_mask.bool(), float('-inf')).type_as(all_scores)
        #print(chn_scores[0])
        #print(cpp_scores[0])

        dpr = loss_fct(all_scores, labels)
        chn = loss_fct(chn_scores, labels)
        cpp = loss_fct(cpp_scores, cf_labels)

        return dpr + taus[0] * chn + taus[1] * cpp

def mhop_loss_cf(model, batch, args):
    ...
    cf_scores_2 = torch.mm(outputs['q_sp1'], cf_ctx.t())
    ...
    cf_scores_1 = cf_scores_1.float().masked_fill(scores_1_mask.bool(), float('-inf')).type_as(cf_scores_1)
    scores_1_hop = torch.cat([scores_1_hop, cf_scores_1, neg_scores_1], dim=1)
    scores_2_hop = torch.cat([scores_2_hop, cf_scores_2, neg_scores_2], dim=1)
    ...
    cf_target_1_hop = torch.arange(outputs["q"].size(0)).to(outputs["q"].device) + 2*outputs["q"].size(0)
    cf_target_2_hop = torch.arange(outputs["q"].size(0)).to(outputs["q"].device) + 2*outputs["q"].size(0)

    retrieve_loss = AADPRLoss(loss_fct, scores_1_hop, target_1_hop, cf_target_1_hop) + \
        AADPRLoss(loss_fct, scores_2_hop, target_2_hop, cf_target_2_hop)
