...
class RobertaRetriever(nn.Module):
    ...
    def forward(self, batch):
        ...
        if 'cf1_input_ids' in batch and 'cf1_mask' in batch:
            cf1 = self.encode_seq(batch['cf1_input_ids'], batch['cf1_mask'])
        else:
            cf1 = None

        if 'cf2_input_ids' in batch and 'cf2_mask' in batch:
            cf2 = self.encode_seq(batch['cf2_input_ids'], batch['cf2_mask'])
        else:
            cf2 = None

        vectors = {'q': q, 'c1': c1, "c2": c2, "neg_1": neg_1, "neg_2": neg_2, "q_sp1": q_sp1, 'cf1': cf1, 'cf2': cf2}
        return vectors
