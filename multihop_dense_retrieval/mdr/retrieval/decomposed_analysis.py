...
def decomposed_errors():
    top1_pred = [json.loads(l) for l in open("/DATAPATH/data/hotpot/dense_val_b1_top1.json").readlines()]
    analysis_folder = "/DATAPATH/data/hotpot/analysis"
...
def collect_gold_decomposition():
    ...
    """
    dev_qdmr = [json.loads(l) for l in open("/DATAPATH/data/QDMR/dev.json").readlines()]
    ...
    with open("/DATAPATH/data/QDMR/inspect.json", "w") as g:
...
def qdmr_utils():
    ...
    """
    qdmr_data = [json.loads(l) for l in open("/DATAPATH/data/QDMR/inspect.json").readlines()]
    ...
    with open("/DATAPATH/data/QDMR/qdmr_e2e.json", "w") as g:
        for item in mhop_data:
            g.write(json.dumps(item) + "\n")


def analyze_results():
    decomposed_results = [json.loads(l) for l in open("/DATAPATH/data/QDMR/qdmr_decomposed_results.json")]
    e2e_results = [json.loads(l) for l in open("/DATAPATH/data/QDMR/qdmr_e2e_results.json")]
    better = 0
    worse = 0
    both = 0
