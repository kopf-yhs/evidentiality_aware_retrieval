...
def load_annotated(path="/PATH/data/hotpot/tfidf/abstracts.txt"):
...
def explore_QDMR(path="/PATH/data/Break-dataset/QDMR-high-level"):
    """
    ...
    hotpot_retrieval_train = [json.loads(l) for l in open("/PATH/data/hotpot/hotpot_train_with_neg_v0.json").readlines()]
    hotpot_retrieval_val = [json.loads(l) for l in open("/PATH/data/hotpot/hotpot_val_with_neg_v0.json").readlines()]
    qid2data = {}
    ...
    for split in ["train", "dev"]:
        ...
        with open(f"/PATH/data/QDMR/{split}.json", "w") as out:
            for _ in break_data:
...
def add_sents_to_corpus_dict():
    ...
    title_and_sents = [json.loads(l) for l in open("/PATH/data/hotpot/tfidf/title_sents.txt").readlines()]
    title2sents = {_['title']:_['sents'] for _ in title_and_sents}
    ...
    json.dump(id2doc, open("/PATH/Mhop-Pretrain/retrieval/index/hotpotQA_corpus_dict.json", "w"))
