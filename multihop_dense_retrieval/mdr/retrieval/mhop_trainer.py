...
from .data.mhop_dataset import MhopDataset, mhop_collate, CfMhopDataset, mhop_collate_cf
from .models.mhop_retriever import (MhopRetriever, RobertaRetriever)
from .criterions import (mhop_loss, mhop_loss_cf, mhop_eval)
...
class Trainer:
    ...
    def _init_state(self) -> None:
        ...
        if self._train_cfg.counterfactual:
            collate_fc = partial(mhop_collate_cf, pad_id=tokenizer.pad_token_id)
            train_set = CfMhopDataset(tokenizer, self._train_cfg.train_file,  self._train_cfg.max_q_len, self._train_cfg.max_q_sp_len, self._train_cfg.max_c_len, train=True)
        else:
    ...
    def _train(self) -> Optional[float]:
        ...
        for epoch in range(start_epoch, self._train_cfg.num_train_epochs):
            ...
            for batch in self._train_loader:
                ...
                loss = loss_function(self._state.model, batch, self._train_cfg)
