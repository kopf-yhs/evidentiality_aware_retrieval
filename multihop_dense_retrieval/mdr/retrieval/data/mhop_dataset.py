...
class MhopDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.data = [json.loads(line.strip()) for line in open(data_path).readlines()]
        '''
        if train:
...
class CfMhopDataset(Dataset):
    ...
    def encode_para(self, para, max_len):
        if para['text'].strip():
            ...
            return self.tokenizer.encode_plus(para["title"].strip(), max_length=max_len, return_tensors="pt")
    ...
    def __getitem__(self, index):
        ...
        neg_codes_1 = self.encode_para(sample["neg_paras"][0], self.max_c_len)
        neg_codes_2 = self.encode_para(sample["neg_paras"][1], self.max_c_len)
        cf_codes_1 = self.encode_para(sample['cf_paras'][0], self.max_c_len)
        if len(sample['cf_paras']) > 1:
            cf_codes_2 = self.encode_para(sample['cf_paras'][1], self.max_c_len)
        else:
            cf_codes_2 = None
        ...
        sample_dict = {
            ...
            "neg_codes_1": neg_codes_1,
            "neg_codes_2": neg_codes_2,
            'cf_codes_1': cf_codes_1,
            'cf_codes_2': cf_codes_2,
        }
...
def mhop_collate_cf(samples, pad_id=0):
    if len(samples) == 0:
        return {}
    batch = {
            ...
            'cf1_input_ids': collate_tokens([s['cf_codes_1']['input_ids'] for s in samples], 0),
            'cf1_mask': collate_tokens([s['cf_codes_1']['attention_mask'] for s in samples], 0),

            'cf2_input_ids': collate_tokens([s['cf_codes_2']['input_ids'] for s in samples], 0),
            'cf2_mask': collate_tokens([s['cf_codes_2']['attention_mask'] for s in samples], 0),
        }
    ...
    if "token_type_ids" in samples[0]["q_codes"]:
        batch.update({
            ...
            'cf1_input_ids': collate_tokens([s["cf_codes_1"]["token_type_ids"] for s in samples], 0),
            'cf2_input_ids': collate_tokens([s["cf_codes_2"]["token_type_ids"] for s in samples], 0)
        })
