...
class EmDataset(Dataset):
    ...
    def __init__(self,
        ...
        if not os.path.exists(save_path):
            ...
            print(f'Making directory : {str(Path(save_path).parent)}')
            Path(save_path).parent.mkdir(exist_ok=True)
        id2doc_save_path = Path(save_path).parent.joinpath('id2doc.json')
        #os.path.join(save_path, "id2doc.json") # ID to doc mapping
        ...
        if self.is_query_embed:
            ...
            if preprocessed:
                print(f"load {len(self.data.keys())} documents...")
                id2doc = self.data
                self.data = [(k,v) for k,v in id2doc.items()]
                ...
                with open(id2doc_save_path, "w") as g:
                    json.dump(id2doc, g)
