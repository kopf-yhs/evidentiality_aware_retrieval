...
from dpr.models.biencoder import BiEncoderNllLoss, BiEncoderNllLossWithCounterfactuals, BiEncoderNllLossForAblation, BiEncoderBatch
...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        bm25_model_file = get_bm25_model_file(cfg)
        if bm25_model_file:
            bm25_saved_state = load_states_from_checkpoint(bm25_model_file)
            set_cfg_params_from_state(saved_state.encoder_params, bm25_saved_state.encoder_params, cfg)'''
    ...
    def run_train(self):
        ...
        #logger.info("Total train epoch: ",int(cfg.train.num_train_epochs))
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            loss, correct_cnt, _, _ = _do_biencoder_fwd_pass(
                self.biencoder,
                biencoder_input,
                self.tensorizer,
                cfg,
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            if cfg.counterfactual:
                biencoder_batch = biencoder.create_biencoder_input_for_counterfactual(
                    samples_batch,
                    self.tensorizer,
                    True,
                    num_hard_negatives,
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int) -> str:
        ...
        cp = cfg.checkpoint_file_name + "." + str(epoch)
...
def _calc_loss(
    ...
    local_cf_negatives_idxs: list = None,
    ...
    if distributed_world_size > 1:
        ...
        cf_negative_per_question = []

        total_ctxs = 0

        for i, item in enumerate(global_question_ctx_vectors):
            q_vector, ctx_vectors, positive_idx, hard_negatives_idxs, cf_negative_idxs = item
            ...
            if i != cfg.local_rank:
                ...
                if local_cf_negatives_idxs:
                    cf_negative_per_question.extend([v + total_ctxs for v in cf_negative_idxs])
            else:
                ...
                if local_cf_negatives_idxs:
                    cf_negative_per_question.extend([v + total_ctxs for v in local_cf_negatives_idxs])
        ...
        if local_cf_negatives_idxs:
            cf_negative_per_question = local_cf_negatives_idxs

    logger.info(global_ctxs_vector.shape)

    if cfg.counterfactual and isinstance(loss_function, BiEncoderNllLossWithCounterfactuals) and local_cf_negatives_idxs:
...
def _do_biencoder_fwd_pass(
    ...
    local_q_vector, local_ctx_vectors, local_q_attentions, local_ctx_attentions = model_out
    #local_q_vector, local_ctx_vectors, local_q_attentions, local_ctx_attentions, local_q_hidden, local_ctx_hidden = model_out

    if model.training and cfg.counterfactual:
        if cfg.ablation:
            loss_function = BiEncoderNllLossForAblation()
        else:
            loss_function = BiEncoderNllLossWithCounterfactuals()
    else:
        loss_function = BiEncoderNllLoss()
    logger.info('Loss function : ' + str(type(loss_function)))
    ...
    if cfg.output_attentions:
        return loss, is_correct, local_q_attentions, local_ctx_attentions
    else:
        return loss, is_correct, None, None
    '''
    return loss, is_correct, local_q_attentions, local_ctx_attentions
    '''
...
#@hydra.main(config_path="conf", config_name="biencoder_train_cfg_sample")
